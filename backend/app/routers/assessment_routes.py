...
from app.services.openai_review import confidence_plain, openai_review_service
...
def _persist_review_result(
    ...
    *,
    ...
    if parsed.is_sufficient and parsed.internal_score is not None:
        ...
        try:
            row.evidence_notes_json = json.dumps(list(parsed.evidence_notes or []), ensure_ascii=False)
...
def export_zip(session_id: int, db: Session = Depends(get_db), definition=Depends(get_assessment_definition)):
    ...
    results = build_results_payload(definition, s, rmap, partial_export=False)
    s.export_payload_json = json.dumps(results, ensure_ascii=False)
    pdf_bytes = build_pdf_report(definition, s, rmap, results)
    zip_path = write_export_zip(s.id, pdf_bytes, results)
    db.commit()
    return FileResponse(
        path=str(zip_path),
...
def export_partial_zip(
    ...
    results = build_results_payload(definition, s, rmap, partial_export=partial_flag)
    s.export_payload_json = json.dumps(results, ensure_ascii=False)
    pdf_bytes = build_pdf_report(definition, s, rmap, results)
    zip_path = write_export_zip(s.id, pdf_bytes, results)
    db.commit()
